        ...
        try:
            ...
            if execution_mode == 'local':
                ...
                return Response({
                    ...
                    'message': f'测试任务已下发到本地执行器：{assigned_runner.name}'
                })
...
class TestCaseExecutionViewSet(viewsets.ModelViewSet):
    ...
    queryset = TestCaseExecution.objects.all().select_related(
        'test_case', 'project', 'test_suite', 'created_by', 'assigned_runner'
    )
    ...
    ordering = ['-created_at']
    filterset_fields = ['project', 'test_suite', 'test_case', 'status', 'execution_source', 'execution_mode']
    pagination_class = StandardPagination
