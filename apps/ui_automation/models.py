...
class TestCaseExecution(models.Model):
    ...
    execution_mode = models.CharField(max_length=20, choices=MODE_CHOICES, default='server', verbose_name='执行模式')
...
class LocalRunner(models.Model):
    ...
    platform = models.CharField(max_length=100, blank=True, verbose_name='平台')
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='online', verbose_name='状态')
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='ui_local_runners', verbose_name='所属用户')
    ...
    def is_online(self):
        if not self.last_heartbeat:
            return False
        return (timezone.now() - self.last_heartbeat).total_seconds() <= 30
